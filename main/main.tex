...
\todo[inline]{Cut down a bit on math and formalism}
...
%\mu \psi(\vb*k) = \oldint_\Omega \rd^2k'\,
...
K_S(k, k', \Delta\theta)  
=\sum_{n=-\infty}^\infty b_{n}(k, k')\, \ee^{\ii n\Delta\theta}
=\sum_{n=-\infty}^\infty \ee^{\ii n\theta}\,b_{n}(k, k')\,\ee^{-\ii n\theta'},
\end{equation}
where 
\begin{equation} \label{eq:KS-FS-coef}
b_{n}(k, k') = \frac{1}{2\pi} \int_0^{2\pi} \rd(\Delta\theta)\; 
...
\psi(k, \theta) = \sum_{m=-\infty}^\infty a_m(k)\ee^{\ii m\theta}.
\end{equation}
...
\begin{equation}
\begin{aligned}
\mu\sum_{l=-\infty}^\infty a_l(k)\ee^{\ii l\theta}
% =& \int_{q\Gamma}^\Gamma\rd{k'}\,k'
% \int_{0}^{2\pi}\rd\theta'\,
% \sum_{n=-\infty}^\infty b_n(k')\ee^{\ii n\theta}\ee^{-\ii n\theta'}
% \sum_{m=-\infty}^\infty b_m(k')\ee^{\ii m\theta'}\\ 
=& \sum_{n=-\infty}^\infty \ee^{\ii n\theta} \sum_{m=-\infty}^\infty 
\int_{q\Gamma}^\Gamma\rd{k'}\,k' b_n(k, k') a_m(k')
\int_{0}^{2\pi}\rd\theta'\,
\ee^{\ii (m-n)\theta'}.
\end{aligned}
\end{equation}
...
\int_{q\Gamma}^\Gamma\rd{k'}\,k' b_n(k, k') a_n(k').
\end{equation}
By the uniqueness of Fourier series expansions, for all non-zero $a_n$
we must have 
\begin{equation}
\label{eq:eigen_fourier_coef}
\mu_n b_n(k) = 2\pi\int_{q\Gamma}^\Gamma\rd{k'}\,k' b_n(k, k') a_n(k'),
\end{equation}
where $\mu$ has been denoted by $\mu_n$ since each $a_n(k)$
will have a different eigenvalue satisfying
\eqref{eq:eigen_fourier_coef}.  
%The problem of finding the largest eigenvalue to \eqref{eq:eigen} now
%results in finding the largest of the largest eigenvalues to
%\eqref{eq:eigen_fourier_coef} for all $n$. 

There is one last thing we can do to make
\eqref{eq:eigen_fourier_coef} more suitable for numerical solution,
and that is to symmetrisize the kernel $k'b_n(k')$. To do that we
make the substitution $a_n(k)=\phi_n(k)/\sqrt{k}$, which yields
\begin{equation}
\mu_n \phi_n(k) = 2\pi\int_{q\Gamma}^\Gamma\rd{k'}\,
\sqrt{kk'}b_n(k, k') \phi_n(k').
\end{equation}
...
evaluate the $\theta$ integral, we see that $m=n$ and we get a factor
of $2\pi$ like in the previous section. By introducing the kernel
\begin{equation}\label{eq:KS-bessel}
K_S^n(k, k') = \int_{0}^{R_s} \rd{r}\, r J_n(kr) J_n(k'r),
\end{equation}
the energy fraction can be written as
\begin{equation}
\begin{aligned}
A_S &=2\pi \sum_{n=-\infty}^{\infty} 
\int_{q\Gamma}^\Gamma \rd{k} \int_{q\Gamma}^\Gamma \rd{k'}\,  
kk'\, b_n(k)b_n^*(k') K_S^n(k, k')\\
&=2\pi \sum_{n=-\infty}^{\infty} \ip{I_S^nb_n}{b_n},
\end{aligned}
\end{equation}
% \begin{equation} \label{eq:energy_bessel}
% \begin{aligned}
...
% \end{aligned}
% \end{equation}
where we in the last step introduced the integral operator
\begin{equation}
  \label{eq:int_op_bessel}
[I_S^nb_n](k'):=\int_{q\Gamma}^\Gamma \rd{k}\, k K_S^n(k,k')b_n(k),
\end{equation}
the inner product
\begin{equation}
  \label{eq:in_prod_bessel}
\ip{u}{v} := \int_{q\Gamma}^\Gamma \rd{k}\, k\, u(k) v(k).
\end{equation}
...
\end{equation}
This fraction can be proven to be maximized by the largest eigenvalue to the family of operators $\{I_n\}$, $\mu_\text{max}$.
...
%\begin{equation*}
%  \sum_{n=-\infty}^{\infty}\mu_n \langle b_n, b_n \rangle \leq \mu_{\text{max}}\sum_{n=-\infty}^{\infty}\langle b_n, b_n\rangle=\mu_{\text{max}},
%\end{equation*}
%where $\mu_n$ denotes the largest eigenvalue of $I_S^n$ and $\mu_{\text{max}}$ the largest of these.
%If we choose the $b_n$:s so that
%\begin{equation*}
%  b_n(k)
%  =\begin{cases} &\phi_n \qc \mu_n=\mu_{\text{max}}  \\
%  &0 \qc \text{otherwise}
%  \end{cases}
%\end{equation*}
...
\todo{Formula \#, and check if wrong}\cite{Gradshteyn-Ryzhik}
\begin{equation}
K_S^n(k, k') =
\begin{cases}
R_S\frac{k'J_{n}(R_sk)J_{n-1}(R_sk')-kJ_{n-1}(R_sk)J_{n}(R_sk')}{k^2-k'^2}
\qc &k\neq k'\\
\frac{R_S^2}{2}\qty[ \Big(J_{n}(R_Sk)\Big)^2
