...
\subsection{Theory}
Assuming a harmonic timedependence, the electric field can be represented, in complex notation, by
\begin{equation}
\vb*E(\vb*x,t)=\vb*E(\vb*x)\ee^{-\ii\omega t}%\cos(\omega t).
\end{equation}
...
\subsubsection{Cylindrical wave decomposition in 2 dimensions}
...
\todo[inline]{This train wreck needs some salvaging!}
...
%The problem of finding the largest eigenvalue to \eqref{eq:eigen_fourier} now results in finding the largest of the largest eigenvalues to \eqref{eq:eigen_fourier_coef} for all $n$.
...
\todo[inline]{Needs a lot of love!}
...
\todo[inline]{Dump this computer child's play  after end document}
...
\end{equation}
where $\lambda = \abs{I}\Lambda/N$ and $\mathsf{K}$ is the $N\times N$ matrix
